...
const { createWorkout, retrieveWorkoutById } = require("../workout/handlers")
const { RATING } = require("../../_utils/constants")
const { createAssignedWorkoutQuery, createAssignmentQuery } = require("./queries")
...
const createAssignmentHandler = async (
    ...
    let assignedWorkouts = await createAssignmentWorkoutListHandler(coachId, trainees, workouts, startDate, assignmentId, language)
    let assignmentData = {
        _id: assignmentId,
        coach: coachId,
        trainees: trainees,
        ...
        switch(workout.type.toLowerCase()) {
            case "program":
                for(let traineeId of trainees) {
                    let assignedWorkouts = await createAssignmentWorkoutsFromProgramHanlder(coachId, traineeId, workout.program, startDate, assignmentId)
                    assignedWorkoutsList = assignedWorkoutsList.concat(assignedWorkouts)
                }
                break
            case "workout":
                for(let traineeId of trainees) {
                    let assignedWorkout = await createAssignmentWorkoutsFromWorkoutHanlder(coachId, traineeId, workout.workout, workout.date, assignmentId)
                    assignedWorkoutsList.push(assignedWorkout)
                }
                break
            case "exercise":
                for(let traineeId of trainees) {
                    let assignedWorkout = await createAssignmentWorkoutsFromExerciseHanlder(coachId, traineeId, workout.exercise, workout.quantity, workout.weight, workout.date, assignmentId, language)
                    assignedWorkoutsList.push(assignedWorkout)
                }
                break
        }
